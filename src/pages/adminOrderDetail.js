        ...
        <tbody>
          ${items
            .map(
              (item) => `
            <tr>
              <td>${escapeHtml(item.productName)}${item.variantLabel ? `<span class="admin-table__meta">${escapeHtml(item.variantLabel)}</span>` : ""}${item.variantIsbnSnapshot ? `<span class="admin-table__meta">ISBN: ${escapeHtml(item.variantIsbnSnapshot)}</span>` : ""}</td>
